...
export default class StudentUI {
    ...
    constructor(socket) {
        ...
        socket.on('openThermolab', () => {
            switchTo(whiteboard, 'up')
            $('iframe.whiteboard').attr('src', "https://virtlabs.tech/apps/mechanics/05_demo/simulator.html")
            $('.menu-content').hide()
            //openThermolab()
        })
        socket.on('openComplab', () => {
            switchTo(computerlab, 'up')
            $('iframe.computerlab').attr('src', "../laboratory/cmplab.html")
            $('.menu-content').hide()
            //openThermolab()
        })
